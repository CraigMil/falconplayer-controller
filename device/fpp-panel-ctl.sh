...
DISPLAY_SERVICES="fpp-worldclock.service fpp-scoreboard.service"

case "$#" in
  1) SVCKEY=worldclock; ACTION="$1" ;;
  2) SVCKEY="$1";       ACTION="$2" ;;
  *) echo "usage: $0 [worldclock|scoreboard|current] {start|stop|restart|enable|disable|status}" >&2; exit 1 ;;
esac
